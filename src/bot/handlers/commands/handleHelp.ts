...
\nБот работает через официальный API ChatGPT от OpenAI последней версии.
\nОбновление лимитов каждый день.
\nМы предоставляем 2 версии подписки:
1. Базовая - доступная всем
2. Расширенная - предоставляет полный доступ ко всем услугам. Стоимость: 299рублей.Вы можете приобрести подписку по команде /pay
\n\*Возможности бота\*
\nКонтекст:
По умолчанию бот работает в режиме контекста, то есть запоминает предыдущие сообщения. Команда /reset сбрасывает контекст.
- Перейдите с помощью команды /mode на одну из моделей по генерации изображений и задайте свой вопрос
- Генерация доступна на русском и английском языках у соответствующих моделей.
\n\*Лимиты и подписка\*
\nБесплатно: 
...
Написать в поддержку — @A_Foggy. Режим работы: 10:00 - 20:00 по мск.`;
  ...
  await bot.sendMessage(message.chat.id, textHelp, { parse_mode: "MarkdownV2" });
};
